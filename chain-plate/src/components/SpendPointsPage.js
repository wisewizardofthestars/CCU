...
function SpendPointsPage({
  ...
  useEffect(() => {
    const fetchChainies = async () => {
      try {
        const userId = localStorage.getItem("currentUserId");
        if (userId) {
          const response = await fetch(`http://localhost:3001/users/${userId}`);
          const data = await response.json();
          setCurrentPoints(data.chainies || 0);
        }
      } catch (error) {
        console.error("Error fetching chainies:", error);
      }
      setLoading(false);
    };
  ...
  const handleRedeem = async (voucher) => {
    try {
      ...
      // Deduct chainies from user's account
      const newBalance = currentPoints - voucher.points;
      const response = await fetch(`http://localhost:3001/users/${userId}`, {
        method: "PATCH",
        headers: {
  ...
  const vouchers = [
    {
      id: 1,
      name: "€5 Farm Discount",
      points: 250,
      description: "Valid on any farm product",
    },
    {
      id: 2,
      name: "€10 Organic Voucher",
      points: 500,
    ...
    {
      id: 4,
      name: "€15 Premium Voucher",
      points: 1000,
      description: "All premium products",
    },
    {
      id: 5,
      name: "30% Off Vegetables",
      points: 1250,
      description: "Discount on all veggies",
    },
  ];
    ...
    <div className="w-full max-w-[360px] mx-auto h-screen bg-white relative flex flex-col font-['Outfit']">
     {/* Status Bar */}
      ...
      <div className="w-full h-[60px] bg-white flex items-center justify-between px-4 border-b border-[#D9D9D9] flex-shrink-0">
        ...
        <h1 className="text-[#404040] text-[20px] font-semibold">
          Spend Chainies
        </h1>
      ...
      {/* Points Display */}
      <div className="w-full bg-gradient-to-r from-[#45ADA1] to-[#3d9a8f] px-6 py-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-white/80 text-[12px]">Your Balance</p>
            <p className="text-white text-[28px] font-bold">
              {currentPoints} Chainies
            </p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[#F5F5F5] pb-20">
        <div className="px-4 py-6">
          <p className="text-gray-600 text-[14px] mb-4">Available Rewards</p>
          <div className="space-y-3">
            {vouchers.map((voucher) => (
              <div
                key={voucher.id}
                className="bg-white rounded-2xl shadow-md p-4 flex items-center gap-4 hover:shadow-lg transition hover:scale-[1.02] cursor-pointer"
              >
                <div className="w-16 h-16 bg-gradient-to-br from-[#45ADA1] to-[#3d9a8f] rounded-xl flex items-center justify-center flex-shrink-0">
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
                    <path
                      d="M20 12V22H4V12M2 7H22V12H2V7ZM12 22V7M12 7H7.5C6.5 7 6 6 6 5.5C6 5 6.5 4 7.5 4H12M12 7H16.5C17.5 7 18 6 18 5.5C18 5 17.5 4 16.5 4H12"
                      stroke="white"
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                ...
                <div className="flex-1">
                  <h3 className="text-[#404040] text-[16px] font-semibold">
                    {voucher.name}
                  </h3>
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-[#45ADA1] text-[14px] font-bold">
                      {voucher.points} pts
                    </span>
                  </div>
                </div>
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-[14px] font-semibold transition ${
                    currentPoints >= voucher.points
                      ? "bg-[#45ADA1] text-white hover:bg-[#3d9a8f]"
                      : "bg-gray-200 text-gray-400 cursor-not-allowed"
                  }`}
                  disabled={currentPoints < voucher.points}
                >
                  {currentPoints >= voucher.points ? "Redeem" : "Locked"}
                </button>
              </div>
            ))}
          </div>
      ...
      {showRedeemPopup && redeemedVoucher && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-[20px] p-8 max-w-[300px] mx-4 text-center shadow-2xl">
            ...
            <div className="font-outfit text-[20px] font-bold text-[#45ADA1] mb-2">
              {redeemedVoucher.name}
            </div>
            <div className="font-outfit text-[16px] text-gray-600 mb-4">
              New balance: {currentPoints} Chainies
            </div>
            <button
